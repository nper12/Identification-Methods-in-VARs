...
for(i in 1: dim(irf)[3]){
  irf[, , i] <- irf[, , i] %*%  P%*%solve(D) #this is how you get one unit shock impulse responses! - one unit shock to y!!! in our case carbon shock
}
...
irf
structural_shock <- array(NA, dim=c(240,1))
structural_shock[4:240,] <- structural_shocks[1,]
structural_shock <- as.data.frame(structural_shock)
structural_shock <- as.data.frame(structural_carbon)
irf_carbon <- as.data.frame(irf_carbon)
rownames(irf_carbon) <- c("Carbon", "Output", "Unemployment", "Inflation", "Interest rate")
