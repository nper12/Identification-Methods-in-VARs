...
\end{aligned}
\end{equation}
In this way, the Phi coefficients or the reduced-form impulse responses can be calculated. Another way to do this is to construct the companion matrix (omitting the constant!):
\[
...
\textbf{3. VARX}
\end{center} 
\noindent The point of a VARX is that one does not need matrix manipulation to identify the structural coefficient. We use an exogenous variable $x$ and put it directly into the reduced-form VAR. Hence, in companion form, the model looks like the following (where again we omit the constant): 
\[
...
$\mathcal{B}$ has the dimensions $(Kp \times Ms)\times M$ and holds the contemporaneous effects of exogenous variables in the system - the most important ones. Again, doing a recursive substitution of $Y_{t}$, we get: 
\[
Y_t = \mathcal{Z} (\mathcal{Z} Y_{t-2} + \mathcal{B}x_{t-1} + U_{t-1}) + \mathcal{B}x_{t} + U_t,
\]
which is equal to:
\[
Y_t = \mathcal{Z}^2Y_{t-2} + \mathcal{Z}\mathcal{B}x_{t-1} +\mathcal{Z} U_{t-1} + \mathcal{B}x_{t} + U_t
\]
and at $h$ step results in:
\[
Y_{t} = \mathcal{Z}^h Y_{t-h} + \sum_{i=0}^{h-1} \mathcal{Z}^i B x_{t-i} + \sum_{i=0}^{h-1} \mathcal{Z}^i U_{t-i}.
\]
and at $\infty$, the first term disappears (again due to $\mathcal{Z}^\infty$ converging to 0 if $y_t$ is a stable stationary process):
\[
Y_{t} = \sum_{i=0}^{\infty} \mathcal{Z}^i B x_{t-i} + \sum_{i=0}^{\infty} \mathcal{Z}^i U_{t-i}
\]
employing the selection matrix $J$ (now of dimension $K\times(Kp+Ms)$, but of the same shape as before), we can get the coefficient matrices of the exogenous variables as well as our normal MA coefficients:
\[
y_{t} = \sum_{i=0}^{\infty} J\mathcal{Z}^i B x_{t-i} + \sum_{i=0}^{\infty} J\mathcal{Z}^i J'u_{t-i}
\]
